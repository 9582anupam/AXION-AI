...
const GenerateQuiz = () => {
    ...
    const [isAssessmentTypeDropdownOpen, setIsAssessmentTypeDropdownOpen] = useState(false)
    ...
    const assessmentTypes = [
        ...
        { id: "ar", name: "Assertion & Reasoning" },
        { id: "sa", name: "Short Answer" },
        { id: "la", name: "Long Answer" }
    ]
    ...
    const handleAssessmentTypeSelect = (type) => {
        ...
        setIsAssessmentTypeDropdownOpen(false)
    }
    ...
    const handleSubmit = () => {
        ...
        console.log("Assessment type:", assessmentType.id)
            ...
            <div className="bg-slate-950 rounded-xl p-8 border border-cyan-900/30 shadow-lg">
                ...
                <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-slate-300 mb-2 font-medium">Assessment Type</label>
                        <div className="relative">
                            <button
                                className="w-full flex items-center justify-between bg-slate-900 border border-slate-700 hover:border-cyan-500/50 rounded-lg p-4 text-left transition-all"
                                onClick={() => setIsAssessmentTypeDropdownOpen(!isAssessmentTypeDropdownOpen)}
                            >
                                {assessmentType ? (
                                    <div>
                                        <div className="font-medium text-slate-200">{assessmentType.name}</div>
                                    </div>
                                ) : (
                                    <span className="text-slate-400">Select assessment type...</span>
                                )}
                                <ChevronDown
                                    className={`h-5 w-5 text-slate-400 transition-transform ${isAssessmentTypeDropdownOpen ? "rotate-180" : ""}`}
                                />
                            </button>

                            {isAssessmentTypeDropdownOpen && (
                                <div className="absolute z-10 w-full mt-2 bg-slate-900 border border-slate-700 rounded-lg shadow-xl">
                                    {assessmentTypes.map((type) => (
                                        <div
                                            key={type.id}
                                            className="p-3 hover:bg-slate-800 cursor-pointer border-b border-slate-800 last:border-0"
                                            onClick={() => handleAssessmentTypeSelect(type)}
                                        >
